...
python create_mmap.py $INPUT_JP2 $INPUT_DIR && python run_infer.py --gpu='0' --nr_types=6 --type_info_path=type_info.json --batch_size=$BATCH_SIZE --model_mode=fast --model_path=$MODEL_PATH  --nr_inference_workers=$N_INF_WORKERS --nr_post_proc_workers=$N_PP_WORKERS \
